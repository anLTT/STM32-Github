...
#define delay_ms(x) HAL_Delay(x-1)

uint8_t SPI_WriteByte(SPI_TypeDef* SPIx,uint8_t Byte,uint8_t cmd)
{
	uint16_t txdata=(uint16_t)((cmd<<15)|(Byte<<7));
	return HAL_SPI_Transmit (&hspi1,(uint8_t*)(&txdata),1,10);			
} 
/*****************************************************************************
...
uint16_t tdata[3]={0xFFFF},rdata[3]={0xFFFF};
/*****************************************************************************
 * @name       :void LCD_RESET(void)
 * @date       :2018-08-09 
...
{  
	...
	LCD_WR_REG(0x11); //再次退出睡眠模式防止一开始没能正确传输数据
...
//显示 16 个点阵
void transfer_mono_data_16pixel(uint8_t dat1,uint8_t dat2)
{
...
//显示 27 个点阵
void transfer_mono_data_27pixel(uint8_t dat1,uint8_t dat2,uint8_t dat3,uint8_t dat4)
{
...
//显示 30 个点阵
void transfer_mono_data_30pixel(uint8_t dat1,uint8_t dat2,uint8_t dat3,uint8_t dat4)
{
	transfer_mono_data_24pixel(dat1,dat2,dat3); //先显示 24 个点阵
	transfer_mono_data_6pixel(dat4); //再显示 6 个点阵，24+6=30
}
//显示 32 个点阵
void transfer_mono_data_32pixel(uint8_t dat1,uint8_t dat2,uint8_t dat3,uint8_t dat4)
{
	transfer_mono_data_24pixel(dat1,dat2,dat3); //先显示 24 个点阵
...
//显示 33 个点阵
void transfer_mono_data_33pixel(uint8_t dat1,uint8_t dat2,uint8_t dat3,uint8_t dat4,uint8_t dat5)
{
	transfer_mono_data_24pixel(dat1,dat2,dat3); //先显示 24 个点阵
	transfer_mono_data_9pixel(dat4,dat5); //再显示 9 个点阵
}
//显示 48 个点阵
void transfer_mono_data_48pixel(uint8_t dat1,uint8_t dat2,uint8_t dat3,uint8_t dat4,uint8_t dat5,uint8_t dat6)
{
	transfer_mono_data_24pixel(dat1,dat2,dat3); //先显示 24 个点阵
...
void transfer_gray_data_3pixel(uint8_t dat1)
{
	uint8_t gray_data;
...
void transfer_gray_data_12pixel(uint8_t dat1,uint8_t dat2,uint8_t dat3)
{
...
void test_display(uint8_t dat1,uint8_t dat2,uint8_t dat3)
{
	uint16_t i,j;
...
void disp_4gray_240x160(uint8_t *dp)
{
	uint8_t i,j;
	uint8_t dat1,dat2,dat3;
