...
{//发送角度数值给电机,并且电机接收
  ...
  {
    setPIDParam(&pos,cmd_P,cmd_I,cmd_D);
  }

  initPID(&vec,15000,5000);//vec pid output current,input vec??
  if(vec.kp == 0)
  {
    setPIDParam(&vec,0.5,0,0);
  }
  else
  {
    setPIDParam(&vec,cmd_P,cmd_I,cmd_D);
  }

  /* Infinite loop */
  for(;;)
  {
    // target_pos = (local_rc_ctrl->rc.ch[0] * 4095.5) / 660.0 + 4095.5;//[0,8191]-8192/2 ?  so float_pos? 使用遥控器控制
