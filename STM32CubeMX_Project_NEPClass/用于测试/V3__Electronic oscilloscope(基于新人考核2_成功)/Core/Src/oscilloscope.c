...
u8 usart_get_data(char *cmd,float *d1,float *d2,float *d3,float *d4)
{
    u8 flag = 0;
    if(usart_readok == 1)
    {
    if(sscanf(usart_readbuff,"%3s=%f,%f,%f,%f#",cmd,d1,d2,d3,d4)==5)
    {
        flag = 1;
    }
    //清除接收完成标志
 ...
 {
     char u_buff[10];
     float u_d1,u_d2,u_d3,u_d4;
     if(usart_get_data(u_buff,&u_d1,&u_d2,&u_d3,&u_d4))
     {
